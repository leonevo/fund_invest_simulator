...
class investment:
    ...
    def calculate(self):
        ...
        if self.strategy == STRATEGY.EVERY_MONTH:
            ...
            self.invest_times = invest_times

            logger.info("You invest fund %s %d times" % (self.fund_code, invest_times))
            logger.info("Total input: %d" % self.total_principal)
            logger.info("Fund total value: %s, Fund return rate: %s" % (str(self.end_total_value), str(self.fund_return_rate)))
            logger.info("Base value: %s, base return rate: %s" % (str(self.base_compair_total_value), str(self.base_return_rate)))
...
if __name__ == '__main__':
    ...
    #invest2.calculate()
    invest2 = investment(1000, start_date, end_date, '470007')
    invest2.calculate()
